...
// ==========================================
// 2. ระบบสลับหน้าเข้าสู่ระบบ & เมนูนำทาง
// ==========================================
...
// ==========================================
// 3. เริ่มต้น LIFF & ตรวจสอบสิทธิ์ผู้ใช้
// ==========================================
...
// ==========================================
// 4. ระบบค้นหาบ้าน เช็คอิน และ ลงทะเบียนแทน
// ==========================================
function setupCheckinEvents() {
    ...
    // รองรับการกด Enter ค้นหา (Smart Search Input)
...
async function searchHouseForCheckin() {
    ...
    // Smart Search Logic (ถ้านิ้วพิมพ์ "99/9-8" มาในช่องเดียว)
    if(rawInput.includes("-")) {
        const parts = rawInput.split("-");
        hNo = parts[0].trim();
        vNo = parts[1].trim();
        document.getElementById("search-village-no").value = vNo; // อัปเดต dropdown ตาม
    }

    if(!hNo || !vNo) return alert("กรุณาระบุบ้านเลขที่และหมู่บ้านให้ถูกต้อง (เช่น 99/9 หรือ 99/9-8)");
    ...
    } catch(e) { console.error(e); listDiv.innerHTML = "<p style='color: red;'>เกิดข้อผิดพลาดในการดึงข้อมูล</p>";
...
function renderCheckinPetCard(docId, pet) {
    ...
    // แยกระดับสิทธิ์ปุ่มบันทึกวัคซีน (แอดมินกดได้ / อาสาฯ โชว์ล็อก)
    if (currentStaffRole === "admin") {
        if (canUndo) {
            actionButton = `<div style="display: flex; gap: 5px;"><button disabled style="flex: 1; background: rgba(80, 227, 194, 0.2); border: 1px solid #50E3C2; color: #50E3C2; padding: 8px; border-radius: 6px; font-size: 13px;">✅ อัปเดตแล้ว</button><button onclick="undoVaccine('${docId}')" style="background: rgba(255, 107, 107, 0.2); border: 1px solid #ff6b6b; color: #ff6b6b; padding: 8px 12px; border-radius: 6px; font-size: 13px; cursor: pointer;">↺ ยกเลิก</button></div>`;
...
window.updateVaccine = async function(docId) {
    ...
    // บันทึกสถานะเดิมไว้เพื่อทำ Undo
    window.petOriginalState[docId] = { 
        vaccine_status: pet.vaccine_status || "", 
        vaccine_year: pet.vaccine_year || 0, 
        vaccine_brand: pet.vaccine_brand || "", 
        vaccine_lot: pet.vaccine_lot || "", 
        vaccine_exp: pet.vaccine_exp || "", 
        vaccinated_by_admin: pet.vaccinated_by_admin || "" 
    };
    ...
    const newData = { 
        vaccine_status: "ฉีดแล้ว", 
        vaccine_year: currentYear, 
        vaccine_brand: sysConfig.vaccine_brand || "วัคซีนเทศบาล", 
...
// บันทึกการลงทะเบียนแทน (เก็บฟิลด์ครบถ้วนเพื่อส่งรายงาน ROD)
...
// ==========================================
// 5. ส่วนจัดการการตั้งค่าระบบ (แอดมินเท่านั้น)
// ==========================================
...
async function saveSettings() {
    ...
    try {
        ...
        await setDoc(doc(db, "system_config", "main_config"), {
            ...
            nt_start_reg: document.getElementById("cfg-nt-start-reg").value, 
            nt_end_reg: document.getElementById("cfg-nt-end-reg").value, 
            nt_date: document.getElementById("cfg-nt-date").value, 
            nt_location: document.getElementById("cfg-nt-location").value,
